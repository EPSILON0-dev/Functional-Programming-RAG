...
export function ApiKeySettingsDialog({ open, onOpenChange }: ApiKeySettingsDialogProps) {
  ...
  function handleSelectKey(selectedKeyId: string) {
    fetch("/api/auth/keys/selected", {
      ...
      .then(() => keys.refetch())
      .catch((err) => console.error("Error setting selected API key:", err));
  }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Manage your OpenRouter API keys.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleAddApiKey}>
          <FieldGroup>
            <ul className="space-y-1">
              ...
              {Array.isArray(keys.data?.api_keys) && keys.data.api_keys.length > 0 ? (
                keys.data.api_keys.map((key: any) => (
                  <li
                    key={key.id}
              ...
              {Array.isArray(keys.data?.api_keys) &&
                keys.data.api_keys.length > 0 &&
                keys.data.api_keys.every((key: any) => key.id !== keys.data.selected_key_id) && (
